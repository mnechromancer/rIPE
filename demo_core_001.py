...
def main():
    ...
    print(
        f"High altitude state: PO2={high_altitude_state.po2} kPa, altitude={high_altitude_state.altitude}m"
    )
    print(
        f"Sea level state: PO2={sea_level_state.po2} kPa, altitude={sea_level_state.altitude}m"
    )
    ...
    states_equal = all(
        original == restored for original, restored in zip(vector, restored_vector)
    )
