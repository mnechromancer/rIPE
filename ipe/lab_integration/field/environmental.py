...
class EnvironmentalReading:
    ...
    def is_complete(self) -> bool:
        ...
        return (
            ...
            and self.humidity_percent is not None
            and self.pressure_hpa is not None
        )
...
class EnvironmentalDataset:
    ...
    def get_readings_by_location(
        self, center: GPSCoordinate, radius_m: float
    ) -> List[EnvironmentalReading]:
        """Get readings within radius of a location"""
        return [r for r in self.readings if r.location.distance_to(center) <= radius_m]
...
class EnvironmentalDataImporter:
    ...
    def import_weather_station_csv(
        self, filepath: Union[str, Path], station_id: str, dataset_id: str
    ...
    def import_multiple_stations(
        self, data_dir: Union[str, Path], dataset_id: str, file_pattern: str = "*.csv"
    ) -> EnvironmentalDataset:
        """
        ...
        for filepath in data_dir.glob(file_pattern):
            ...
            try:
                dataset = self.import_weather_station_csv(
                    filepath, station_id, f"{dataset_id}_{station_id}"
                )
    ...
    def align_with_gps_track(
        ...
        for reading in dataset.readings:
            ...
            for gps_point in gps_track:
                ...
                time_diff = abs(
                    (reading.timestamp - gps_point.timestamp).total_seconds() / 60
                )
        ...
        for station_id, readings in station_readings.items():
            ...
            for col in df.columns:
                ...
                if method == "linear":
                    df_interp[col] = df[col].interpolate(
                        method="time", limit=int(max_gap_hours * 60)
                    )
                elif method == "nearest":
                    df_interp[col] = df[col].fillna(
                        method="nearest", limit=int(max_gap_hours * 60)
                    )
                else:
                    df_interp[col] = df[col].interpolate(
                        method=method, limit=int(max_gap_hours * 60)
                    )
            ...
            for i, (timestamp, row) in enumerate(df_interp.iterrows()):
                ...
                interpolated_reading = EnvironmentalReading(
                    ...
                    metadata={**original_reading.metadata, "interpolated": True},
                )
    ...
    def _export_csv(
        self, dataset: EnvironmentalDataset, output_path: Path, include_metadata: bool
    ):
