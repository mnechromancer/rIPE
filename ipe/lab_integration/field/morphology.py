...
class MorphometricDataset:
    ...
    def get_measurements_by_trait(self, trait_name: str) -> List[MorphologicalMeasurement]:
        """Get all measurements for a specific trait"""
        return self._trait_index.get(trait_name, [])
    
    def get_trait_statistics(self, trait_name: str, 
...
class MorphologyDataImporter:
    ...
    def import_specimen_csv(self, filepath: Union[str, Path],
                           dataset_id: str) -> MorphometricDataset:
        """
        ...
        for _, row in df.iterrows():
            ...
            for column in df.columns:
                ...
                if column in ['specimen_id', 'species', 'collection_date', 'collection_location',
                             'latitude', 'longitude', 'elevation_m', 'sex', 'age_class',
                             'life_stage', 'collector', 'collection_method', 'preservation_method',
                             'catalog_number', 'museum_code']:
                    continue
                ...
                if value is not None:
                    ...
                    if column not in trait_definitions:
                        trait_definitions[column] = self.standard_traits.get(column, {
    ...
    def import_measurements_csv(self, filepath: Union[str, Path],
        ...
        for _, row in df.iterrows():
            ...
            if specimen_id not in specimens_dict:
                print(f"Warning: Specimen {specimen_id} not found, skipping measurement")
                continue
            ...
            if trait_name not in trait_definitions:
                trait_definitions[trait_name] = self.standard_traits.get(trait_name, {
    ...
    def quality_control(self, dataset: MorphometricDataset,
        ...
        report = {
            ...
            'total_measurements': sum(len(s.measurements) for s in dataset.specimens.values()),
            'issues': [],
            'outliers': {},
            'missing_data': {},
            'summary': {}
        ...
        missing_species = [s.specimen_id for s in dataset.specimens.values() if not s.species]
        if missing_species:
            report['issues'].append(f"Missing species data: {len(missing_species)} specimens")
        ...
        report['summary'] = {
            ...
            'total_outliers': sum(len(outliers) for outliers in report['outliers'].values()),
            'trait_completeness': trait_completeness,
            'mean_completeness': np.mean(list(trait_completeness.values())) if trait_completeness else 0
        }
        
        return report
    
    def export_dataset(self, dataset: MorphometricDataset,
    ...
    def _infer_unit(self, trait_name: str) -> str:
        ...
        if any(keyword in trait_name_lower for keyword in ['mass', 'weight']):
            return 'g'
        elif any(keyword in trait_name_lower for keyword in ['length', 'width', 'height', 'diameter']):
            return 'mm'
        else:
    ...
    def _export_long_csv(self, dataset: MorphometricDataset, output_path: Path, include_statistics: bool):
        ...
        with open(output_path, 'w', newline='') as csvfile:
            ...
            # Header
            header = [
                'specimen_id', 'species', 'trait_name', 'value', 'unit',
                'measurement_method', 'observer', 'date_measured',
                'collection_date', 'collection_location', 'latitude', 'longitude',
                'sex', 'age_class', 'catalog_number', 'museum_code'
            ]
            ...
            for specimen in dataset.specimens.values():
                for measurement in specimen.measurements:
                    row = [
                        ...
                        measurement.date_measured.isoformat() if measurement.date_measured else '',
                        specimen.collection_date.isoformat() if specimen.collection_date else '',
    ...
    def _export_wide_csv(self, dataset: MorphometricDataset, output_path: Path, include_statistics: bool):
        ...
        with open(output_path, 'w', newline='') as csvfile:
            ...
            # Header
            header = [
                'specimen_id', 'species', 'collection_date', 'collection_location',
                'latitude', 'longitude', 'sex', 'age_class', 'catalog_number', 'museum_code'
            ]
    ...
    def _export_json(self, dataset: MorphometricDataset, output_path: Path, include_statistics: bool):
        ...
        for specimen_id, specimen in dataset.specimens.items():
            specimen_data = {
                'species': specimen.species,
                'collection_date': specimen.collection_date.isoformat() if specimen.collection_date else None,
                'collection_location': specimen.collection_location,
                'coordinates': {
                    'latitude': specimen.latitude,
                    'longitude': specimen.longitude,
            ...
            for measurement in specimen.measurements:
                specimen_data['measurements'][measurement.trait_name] = {
                    ...
                    'precision': measurement.precision,
                    'observer': measurement.observer,
                    'date': measurement.date_measured.isoformat() if measurement.date_measured else None,
                    'notes': measurement.notes
                }
