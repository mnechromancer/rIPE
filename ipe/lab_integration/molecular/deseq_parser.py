...
class PathwayAnalysis:
    ...
    def get_significant_pathways(self, p_threshold: float = 0.05) -> List[EnrichmentResult]:
        """Get significantly enriched pathways"""
        return [result for result in self.enrichment_results 
...
class DESeqParser:
    ...
    def parse_deseq2_contrasts(self, results_dir: Union[str, Path],
                              experiment_id: str) -> Dict[str, RNASeqExperiment]:
        """
    ...
    def parse_gsea_results(self, filepath: Union[str, Path],
                          analysis_id: str) -> PathwayAnalysis:
        """
        ...
        for _, row in df.iterrows():
            ...
            enrichment_result = EnrichmentResult(
                ...
                fold_enrichment=float(row.get('NES', 0.0))  # Use NES as fold enrichment
            )
    ...
    def parse_david_results(self, filepath: Union[str, Path],
                           analysis_id: str) -> PathwayAnalysis:
        """
    ...
    def parse_topgo_results(self, filepath: Union[str, Path],
                           analysis_id: str,
                           category: str = 'GO_BP') -> PathwayAnalysis:
        """
        ...
        for _, row in df.iterrows():
            ...
            enrichment_result = EnrichmentResult(
                ...
                fold_enrichment=float(row.get('Significant', 0)) / float(row.get('Expected', 1))
            )
    ...
    def integrate_expression_pathways(self, experiment: RNASeqExperiment,
        ...
        sig_gene_ids = {expr.gene.gene_symbol or expr.gene.gene_id for expr in sig_genes}
        ...
        summary = {
            ...
            'statistics': {
                ...
                'downregulated_genes': len(experiment.get_downregulated_genes(p_threshold)),
    ...
    def export_integrated_results(self, integration_summary: Dict[str, Any],
        ...
        if format.lower() == 'json':
            ...
            with open(output_path, 'w', newline='') as f:
                ...
                # Write header
                writer.writerow([
                    'pathway_name', 'pathway_p_value', 'pathway_p_adj',
                    'total_pathway_genes', 'significant_de_genes', 
                    'overlap_genes'
                ])
                
                # Write pathway data
                for pathway_name, data in integration_summary.get('pathway_overlaps', {}).items():
                    writer.writerow([
                        pathway_name,
                        f"{data['p_value']:.2e}",
                        f"{data['p_adj']:.2e}",
    ...
    def compare_contrasts(self, experiments: Dict[str, RNASeqExperiment],
                         p_threshold: float = 0.05,
                         lfc_threshold: float = 1.0) -> Dict[str, Any]:
        """
        ...
        for contrast_name, experiment in experiments.items():
            ...
            gene_ids = {expr.gene.gene_symbol or expr.gene.gene_id for expr in sig_genes}
        ...
        for i, contrast1 in enumerate(contrast_names):
            for j in range(i + 1, len(contrast_names)):
                ...
                overlap = contrast_genes[contrast1].intersection(contrast_genes[contrast2])
                pair_name = f"{contrast1}_vs_{contrast2}"
                pairwise_overlaps[pair_name] = {
                    'overlap_genes': overlap,
                    'overlap_count': len(overlap),
                    'contrast1_unique': contrast_genes[contrast1] - contrast_genes[contrast2],
                    'contrast2_unique': contrast_genes[contrast2] - contrast_genes[contrast1]
                }
        
        # Find genes common to all contrasts
        common_genes = set.intersection(*contrast_genes.values()) if contrast_genes.values() else set()
