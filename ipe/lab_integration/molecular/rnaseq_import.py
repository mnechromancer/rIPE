...
class ExpressionData:
    ...
    normalized_counts: Optional[Dict[str, float]] = (
        None  # sample_id -> normalized count
    )

    def is_significant(
        self, p_threshold: float = 0.05, lfc_threshold: float = 1.0
    ) -> bool:
        """Check if gene is significantly differentially expressed"""
        if self.p_adj is None:
            return False
        return self.p_adj < p_threshold and abs(self.log2_fold_change) > lfc_threshold
...
class RNASeqExperiment:
    ...
    def __post_init__(self):
        ...
        self._symbol_index = {
            expr.gene.gene_symbol: expr
            for expr in self.expression_data
    ...
    def get_upregulated_genes(
        ...
        return [
            ...
            for expr in self.expression_data
            if (
                expr.is_significant(p_threshold, lfc_threshold)
                and expr.log2_fold_change > 0
            )
        ]

    def get_downregulated_genes(
        self, p_threshold: float = 0.05, lfc_threshold: float = 1.0
    ) -> List[ExpressionData]:
...
class GeneIDMapper:
    ...
    def _load_default_mappings(self):
        ...
        self.mappings = {
            ...
            "ENSMUSG00000000037": {
                "symbol": "Scml2",
                "name": "sex comb on midleg-like 2",
            },
            "ENSMUSG00000000056": {
                "symbol": "Narf",
...
class RNASeqImporter:
    ...
    def export_experiment(
        ...
        if format.lower() in ["csv", "tsv"]:
            self._export_tabular(
                experiment, output_path, format, include_nonsignificant
            )
