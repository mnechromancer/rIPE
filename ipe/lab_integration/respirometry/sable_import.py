...
class SableSystemsImporter:
    ...
    def __init__(self, baseline_correction: bool = True, 
    ...
    def _parse_csv_file(self, filepath: Path) -> SableSessionData:
        ...
        for line in lines:
            ...
            elif line and ',' in line and not line.lower().startswith('time'):  # Skip header row
                data_lines.append(line)
        
        return self._parse_measurements(data_lines, filepath.stem, metadata)
    
    def _parse_measurements(self, data_lines: List[str], 
                           session_id: str, metadata: Dict) -> SableSessionData:
        ...
        for line in data_lines:
            ...
            if len(parts) < 7:  # Need at least time, VO2, VCO2, temp, humidity, pressure, flow
                continue
            
            try:
                # Parse timestamp - assume format like "14:23:15" or "2024-01-01 14:23:15"
                time_str = parts[0]
                if ' ' in time_str:
    ...
    def _apply_temperature_correction(self, vo2: float, vco2: float, 
                                    temp_c: float, pressure_kpa: float) -> tuple:
        """
    ...
    def _apply_baseline_correction(self, session_data: SableSessionData):
        ...
        # For this implementation, we'll assume a small baseline drift rather than full subtraction
    ...
    def export_session(self, session_data: SableSessionData, 
                      output_path: Union[str, Path], format: str = 'csv'):
        """
    ...
    def _export_csv(self, session_data: SableSessionData, output_path: Path):
        ...
        with open(output_path, 'w', newline='') as csvfile:
            ...
            writer.writerow(['Timestamp', 'VO2_ml_min', 'VCO2_ml_min', 'RER', 
                           'Temp_C', 'Humidity_%', 'Pressure_kPa', 'Flow_ml_min',
                           'Baseline_Corrected'])
