...
class GenericRespirometryParser:
    ...
    def filter_measurements(
        ...
        return SableSessionData(
            ...
            start_time=(
                filtered_measurements[0].timestamp
                if filtered_measurements
                else session_data.start_time
            ),
            end_time=(
                filtered_measurements[-1].timestamp
                if filtered_measurements
    ...
    def smooth_measurements(
        self, session_data: SableSessionData, window_size: int = 5
    ) -> SableSessionData:
        """
        ...
        for i in range(len(measurements)):
            ...
            smooth_rer = (
                smooth_vco2 / smooth_vo2 if smooth_vo2 > 0 else measurements[i].rer
            )
    ...
    def recommend_processing(quality_assessment: Dict[str, Any]) -> List[str]:
        ...
        if "Short recording" in " ".join(issues):
            recommendations.append(
                "Consider longer recording duration for better statistics"
            )
