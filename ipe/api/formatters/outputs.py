...
class DataFormatter:
    ...
    def format_data(self, data: Dict[str, Any], format_type: str, output_path: Optional[str] = None) -> Union[str, bytes, io.BytesIO]:
        ...
        with h5py.File(output_path, 'w') as hf:
            for key, value in data.items():
                if isinstance(value, pd.DataFrame):
                    ...
                    grp.attrs['columns'] = [col.encode('utf-8') for col in value.columns]
...
class StatisticalSummaryFormatter:
    ...
    def __init__(self):
        self.summary_fields = [
            ...
            'quantiles', 'skewness', 'kurtosis', 'count'
        ]
            ...
            if isinstance(value, pd.DataFrame):
                ...
                if len(value) > 0:
                    ...
                    if np.issubdtype(arr.dtype, np.number):
                        summary['numerical_summary'][key] = self._analyze_numeric_array(arr)
            elif isinstance(value, dict):
