...
async def generate_publication_figure(
    ...
    if figure_request.figure_type == "state_space":
        ...
        for i in range(len(x_bins)-1):
            for j in range(len(y_bins)-1):
                mask = ((state_df['x_coord'] >= x_bins[i]) & 
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename={simulation_id}_figure.{figure_request.format}"}
    )
...
async def get_statistical_summary(simulation_id: str):
    ...
    summary = {
        ...
        "fitness_distribution": {
            ...
            "quartiles": {str(k): float(v) for k, v in state_df['fitness'].quantile([0.25, 0.5, 0.75]).items()}
        },
        "spatial_distribution": {
            "x_range": [float(state_df['x_coord'].min()), float(state_df['x_coord'].max())],
            "y_range": [float(state_df['y_coord'].min()), float(state_df['y_coord'].max())],
            "z_range": [float(state_df['z_coord'].min()), float(state_df['z_coord'].max())],
        },
...
async def process_export(export_id: str, simulation_id: str, export_request: ExportRequest):
