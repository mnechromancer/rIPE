...
class Organism(Base, TimestampMixin):
    ...
    measurements = relationship("PhysiologyMeasurement", back_populates="organism", cascade="all, delete-orphan")
    ...
    def validate_plasticity(self, key, value):
        ...
        required_keys = ['thermal_sensitivity', 'hypoxia_tolerance', 'metabolic_flexibility']
...
class Simulation(Base, TimestampMixin):
    ...
    # Relationships
    organisms = relationship("Organism", back_populates="simulation", cascade="all, delete-orphan")
    environments = relationship("EnvironmentalCondition", back_populates="simulation", cascade="all, delete-orphan")
...
class EnvironmentalCondition(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint('temperature >= -50 AND temperature <= 70', name='realistic_temperature'),
...
class PhysiologyMeasurement(Base, TimestampMixin):
    ...
    # Metadata
    data_source = Column(String(100), nullable=False)  # 'simulation', 'respirometry', 'field'
...
class GeneticMarker(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint('allele_frequency IS NULL OR (allele_frequency >= 0 AND allele_frequency <= 1)', 
...
class ExperimentalData(Base, TimestampMixin):
    ...
    data_type = Column(String(100), nullable=False)  # 'respirometry', 'rnaseq', 'morphology'
    ...
    linked_organism_id = Column(UUID(as_uuid=True), ForeignKey('organisms.id'), nullable=True)
...
class EvolutionaryEvent(Base, TimestampMixin):
    ...
    # Event details
    event_type = Column(String(100), nullable=False)  # 'speciation', 'extinction', 'adaptation'
