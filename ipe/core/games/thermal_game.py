...
class ThermalGame(GameSpecification):
    def __init__(
        ...
        self.environment_temp = (
            constraints.get("environment_temp", 20.0) if constraints else 20.0
        )

    def compute_payoff(
        self, shivering: float, non_shivering: float, environment: Any
    ) -> float:
        """
        ...
        temp = (
            environment.get("temperature", self.environment_temp)
            if isinstance(environment, dict)
            else getattr(environment, "temperature", self.environment_temp)
        )
        po2 = (
