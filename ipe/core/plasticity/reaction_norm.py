...
class ReactionNorm:
        ...
        try:
            if self.interpolation_method == 'linear':
                self.interpolator = interpolate.interp1d(
                    self.environments, self.phenotypes, 
    ...
    def predict_phenotype(self, environment: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        """
        ...
        try:
            ...
            for i in range(len(second_deriv) - 1):
                if second_deriv[i] * second_deriv[i + 1] < 0:
                    ...
                    env_interp = self.environments[i] + (self.environments[i + 1] - self.environments[i]) * \
                                abs(second_deriv[i]) / (abs(second_deriv[i]) + abs(second_deriv[i + 1]))
