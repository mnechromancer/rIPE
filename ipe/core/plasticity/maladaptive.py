...
class MaladaptiveDetector:
    ...
    fitness_function: Callable[[float, float], float]  # (phenotype, environment) -> fitness
    ...
    def compute_optimal_phenotype(self, environment: float, 
                                  phenotype_range: Optional[Tuple[float, float]] = None) -> float:
        """
    ...
    def detect_maladaptive_responses(self, reaction_norm: ReactionNorm,
                                     environments: Optional[np.ndarray] = None,
                                     phenotype_range: Optional[Tuple[float, float]] = None) -> List[MaladaptiveResponse]:
        """
        ...
        for env in environments:
            ...
            if plastic_fitness < optimal_fitness:
                ...
                relative_cost = fitness_cost / optimal_fitness if optimal_fitness > 0 else 0
    ...
    def quantify_maladaptation_severity(self, reaction_norm: ReactionNorm,
                                        environments: Optional[np.ndarray] = None) -> Dict[str, float]:
        """
    ...
    def compare_plastic_vs_constitutive(self, reaction_norm: ReactionNorm,
                                        constitutive_phenotype: float,
                                        environments: Optional[np.ndarray] = None) -> Dict[str, Union[float, List[float]]]:
        """
        ...
        return {
            ...
            'plastic_advantage': np.mean(plastic_fitness) - np.mean(constitutive_fitness),
            'proportion_plastic_better': np.mean(plastic_fitness > constitutive_fitness),
    ...
    def identify_maladaptive_environments(self, reaction_norm: ReactionNorm,
                                          threshold: float = 0.05,
                                          environments: Optional[np.ndarray] = None) -> List[float]:
        """
    ...
    def compute_plasticity_cost_function(self, reaction_norm: ReactionNorm,
                                          environments: Optional[np.ndarray] = None) -> ReactionNorm:
        """
...
def quadratic_fitness(phenotype: float, environment: float, 
                     optimum_slope: float = 1.0, width: float = 10.0) -> float:
    """
...
def linear_fitness(phenotype: float, environment: float,
                  slope: float = 1.0, intercept: float = 0.0) -> float:
    """
