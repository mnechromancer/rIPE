...
class CanalizationTrajectory:
    ...
    def __post_init__(self):
        ...
        arrays = [
            self.generations,
            self.canalization_strength,
            self.phenotypic_variance,
...
class CanalizationEngine:
    ...
    def measure_genetic_canalization(
        self, reaction_norms: List[ReactionNorm], environment: float
    ) -> CanalizationMeasure:
        """
    ...
    def measure_environmental_canalization(
        self,
    ...
    def measure_developmental_canalization(
        self, mean_phenotype: float, phenotypic_variance: float, trait_name: str
    ) -> CanalizationMeasure:
        """
    ...
    def simulate_canalization_evolution(
        self, initial_variance: float, trait_name: str, generations: int = 1000
        ...
        for gen in range(generations):
            ...
            current_canalization = np.clip(
                current_canalization + canalization_change + noise, 0, 1
            )
    ...
    def predict_canalization_trajectory(
        self, assimilation_trajectory: AssimilationTrajectory
    ) -> CanalizationTrajectory:
        """
    ...
    def compare_canalization_types(
        self,
        reaction_norms: List[ReactionNorm],
        environment: float,
        developmental_variance: float = 0.1,
    ) -> Dict[str, CanalizationMeasure]:
        """
        ...
        if len(reaction_norms) >= 2:
            ...
            genetic_measure = self.measure_genetic_canalization(
                reaction_norms, environment
            )
        ...
        if reaction_norms:
            ...
            total_variance = (
                (genetic_measure.variance_component if "genetic" in measures else 0)
                + env_measure.variance_component
            ...
            phenotypic_measure = CanalizationMeasure(
                ...
                strength=(
                    1.0 / (1.0 + np.sqrt(total_variance)) if total_variance > 0 else 1.0
                ),
                trait_name=reaction_norms[0].trait_name,
                environment=environment,
    ...
    def assess_canalization_evolution(
        self,
        ...
        canalization_values = [
            trajectory.canalization_strength[i]
            for i in time_points
            if i < len(trajectory.canalization_strength)
        ]
        ...
        return {
            ...
            "final_variance_reduction": (
                1
                - trajectory.phenotypic_variance[-1] / trajectory.phenotypic_variance[0]
                if trajectory.phenotypic_variance[0] > 0
                else 0
            ),
            "evolutionary_stability": "stable" if stable else "unstable",
            "canalization_efficiency": (
                final_canalization / max_canalization if max_canalization > 0 else 0
            ),
        }

    def identify_canalization_drivers(
        self, trajectory: CanalizationTrajectory
    ) -> Dict[str, float]:
        """
        ...
        selection_contribution = (
            self.selection_strength * 0.8
        )  # Selection is primary driver

        drift_effect = 1.0 / (2 * self.population_size)
        drift_contribution = min(
            0.3, drift_effect
        )  # Genetic drift opposes canalization

        mutation_contribution = (
            self.mutation_rate * 0.2
        )  # Mutation opposes canalization

        developmental_contribution = max(
            0, 0.5 - self.developmental_noise
        )  # Low noise favors canalization
