...
class GxEInteraction:
    ...
    def __post_init__(self):
        ...
        if len(env_vars) > 1:
            raise ValueError(
                f"All genotypes must use same environmental variable, found: {env_vars}"
            )
        if len(traits) > 1:
    ...
    def evaluate_at_environments(
        self, environments: np.ndarray
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def specialist_vs_generalist(self) -> Dict[str, str]:
        ...
        for genotype_id in self.genotypes.keys():
            ...
            cv = (
                np.std(phenotypes) / np.abs(np.mean(phenotypes))
                if np.mean(phenotypes) != 0
                else 0
            )
    ...
    def environmental_canalization(
        self, environments: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
        """
