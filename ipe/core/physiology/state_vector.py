...
class StateVector:
    ...
    def euclidean_distance(self, other: Union['StateVector', PhysiologicalState]) -> np.ndarray:
        """
    ...
    def manhattan_distance(self, other: Union['StateVector', PhysiologicalState]) -> np.ndarray:
        """
    ...
    def mahalanobis_distance(self, other: Union['StateVector', PhysiologicalState], 
    ...
    def _state_to_vector(self, state: PhysiologicalState) -> np.ndarray:
        ...
        numeric_fields = [
            'po2', 'temperature', 'altitude', 'heart_mass', 'hematocrit', 
            'hemoglobin', 'blood_volume', 'cardiac_output', 'lung_volume',
            'diffusion_capacity', 'ventilation_rate', 'tidal_volume', 'bmr',
            'vo2max', 'respiratory_exchange_ratio', 'thermal_conductance',
            'lower_critical_temp', 'upper_critical_temp', 'max_thermogenesis'
        ]
        ...
        optional_fields = ['salinity', 'plasma_osmolality', 'gill_na_k_atpase', 'drinking_rate']
        for field in optional_fields:
            value = getattr(state, field)
            vector_parts.append(value if value is not None else 0.0)
            
        # Mitochondrial density (convert dict to fixed-order vector)
        tissue_order = [Tissue.BRAIN, Tissue.HEART, Tissue.MUSCLE, Tissue.BROWN_FAT, 
                       Tissue.KIDNEY, Tissue.LUNG, Tissue.GILL]
    ...
    def _state_to_dict(self, state: PhysiologicalState) -> Dict[str, Any]:
        ...
        if 'mitochondrial_density' in state_dict and state_dict['mitochondrial_density']:
    ...
    def _vector_to_state(self, vector: np.ndarray) -> PhysiologicalState:
        ...
        # Basic numerical fields  
        po2 = vector[i]; i += 1
        temperature = vector[i]; i += 1
        altitude = vector[i]; i += 1
        heart_mass = vector[i]; i += 1
        hematocrit = vector[i]; i += 1
        hemoglobin = vector[i]; i += 1
        blood_volume = vector[i]; i += 1
        cardiac_output = vector[i]; i += 1
        lung_volume = vector[i]; i += 1
        diffusion_capacity = vector[i]; i += 1
        ventilation_rate = vector[i]; i += 1
        tidal_volume = vector[i]; i += 1
        bmr = vector[i]; i += 1
        vo2max = vector[i]; i += 1
        respiratory_exchange_ratio = vector[i]; i += 1
        thermal_conductance = vector[i]; i += 1
        lower_critical_temp = vector[i]; i += 1
        upper_critical_temp = vector[i]; i += 1
        max_thermogenesis = vector[i]; i += 1
        
        # Optional fields
        salinity = vector[i] if vector[i] != 0 else None; i += 1
        plasma_osmolality = vector[i] if vector[i] != 0 else None; i += 1
        gill_na_k_atpase = vector[i] if vector[i] != 0 else None; i += 1
        drinking_rate = vector[i] if vector[i] != 0 else None; i += 1
