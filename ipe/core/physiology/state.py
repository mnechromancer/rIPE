...
class PhysiologicalState:
    ...
    mitochondrial_density: Optional[Dict[Tissue, float]] = None  # relative, range: 0.5-2.0
    ...
    def __post_init__(self):
        ...
        if self.mitochondrial_density is None:
            ...
            object.__setattr__(self, 'mitochondrial_density', {
                Tissue.HEART: 1.5,
                Tissue.MUSCLE: 1.0,
                Tissue.BRAIN: 1.8,
        ...
        if self.tissue_perfusion is None:
            ...
            object.__setattr__(self, 'tissue_perfusion', {
                Tissue.BRAIN: 0.15,
                Tissue.HEART: 0.04,
                Tissue.KIDNEY: 0.22,
                Tissue.MUSCLE: 0.40,
    ...
    def __hash__(self):
        ...
        mito_tuple = tuple(sorted(self.mitochondrial_density.items(), key=lambda x: x[0].value)) if self.mitochondrial_density else None
        perfusion_tuple = tuple(sorted(self.tissue_perfusion.items(), key=lambda x: x[0].value)) if self.tissue_perfusion else None
        
        # Create tuple of all hashable values
        hash_tuple = (
            self.po2, self.temperature, self.altitude, self.salinity,
            self.heart_mass, self.hematocrit, self.hemoglobin, self.blood_volume, self.cardiac_output,
            self.lung_volume, self.diffusion_capacity, self.ventilation_rate, self.tidal_volume,
            self.bmr, self.vo2max, self.respiratory_exchange_ratio,
            self.thermal_conductance, self.lower_critical_temp, self.upper_critical_temp, self.max_thermogenesis,
            mito_tuple, perfusion_tuple,
            self.plasma_osmolality, self.gill_na_k_atpase, self.drinking_rate
        )
    ...
    def validate(self) -> None:
        ...
        if not (-40.0 <= self.temperature <= 50.0):
            raise ValueError(f"temperature {self.temperature}°C outside valid range -40 to 50°C")
        ...
        if self.salinity is not None and not (0.0 <= self.salinity <= 35.0):
            raise ValueError(f"salinity {self.salinity} ppt outside valid range 0-35 ppt")
            
        # Cardiovascular validation
        if not (3.0 <= self.heart_mass <= 15.0):
            raise ValueError(f"heart_mass {self.heart_mass} g/kg outside valid range 3-15 g/kg")
        if not (20.0 <= self.hematocrit <= 70.0):
            raise ValueError(f"hematocrit {self.hematocrit}% outside valid range 20-70%")
        if not (10.0 <= self.hemoglobin <= 25.0):
            raise ValueError(f"hemoglobin {self.hemoglobin} g/dL outside valid range 10-25 g/dL")
        if not (50.0 <= self.blood_volume <= 120.0):
            raise ValueError(f"blood_volume {self.blood_volume} mL/kg outside valid range 50-120 mL/kg")
        if not (100.0 <= self.cardiac_output <= 500.0):
            raise ValueError(f"cardiac_output {self.cardiac_output} mL/min/kg outside valid range 100-500 mL/min/kg")
            
        # Respiratory validation
        if not (40.0 <= self.lung_volume <= 100.0):
            raise ValueError(f"lung_volume {self.lung_volume} mL/kg outside valid range 40-100 mL/kg")
        if not (1.0 <= self.diffusion_capacity <= 5.0):
            raise ValueError(f"diffusion_capacity {self.diffusion_capacity} outside valid range 1-5")
        if not (10.0 <= self.ventilation_rate <= 100.0):
            raise ValueError(f"ventilation_rate {self.ventilation_rate} breaths/min outside valid range 10-100")
        if not (5.0 <= self.tidal_volume <= 15.0):
            raise ValueError(f"tidal_volume {self.tidal_volume} mL/kg outside valid range 5-15 mL/kg")
        ...
        if not (30.0 <= self.vo2max <= 200.0):
            raise ValueError(f"vo2max {self.vo2max} mL O2/min/kg outside valid range 30-200")
        if not (0.7 <= self.respiratory_exchange_ratio <= 1.0):
            raise ValueError(f"respiratory_exchange_ratio {self.respiratory_exchange_ratio} outside valid range 0.7-1.0")
            
        # Thermoregulation validation
        if not (1.0 <= self.thermal_conductance <= 5.0):
            raise ValueError(f"thermal_conductance {self.thermal_conductance} outside valid range 1-5")
        if not (-10.0 <= self.lower_critical_temp <= 25.0):
            raise ValueError(f"lower_critical_temp {self.lower_critical_temp}°C outside valid range -10 to 25°C")
        if not (25.0 <= self.upper_critical_temp <= 45.0):
            raise ValueError(f"upper_critical_temp {self.upper_critical_temp}°C outside valid range 25 to 45°C")
        if not (50.0 <= self.max_thermogenesis <= 300.0):
            raise ValueError(f"max_thermogenesis {self.max_thermogenesis} outside valid range 50-300")
            
        # Temperature logic validation
        if self.lower_critical_temp >= self.upper_critical_temp:
            raise ValueError(f"lower_critical_temp {self.lower_critical_temp}°C must be < upper_critical_temp {self.upper_critical_temp}°C")
        ...
        if self.mitochondrial_density:
            for tissue, density in self.mitochondrial_density.items():
                if not (0.5 <= density <= 2.0):
                    raise ValueError(f"mitochondrial_density for {tissue.value} outside valid range 0.5-2.0")
        ...
        if self.tissue_perfusion:
            ...
            for tissue, fraction in self.tissue_perfusion.items():
                if not (0.0 <= fraction <= 1.0):
                    raise ValueError(f"tissue_perfusion for {tissue.value} outside valid range 0-1")
            ...
            if not (0.95 <= total_perfusion <= 1.05):  # Allow small rounding errors
                raise ValueError(f"tissue_perfusion fractions sum to {total_perfusion}, should sum to ~1.0")
                    
        # Osmoregulation validation (if present)
        if self.plasma_osmolality is not None and not (250.0 <= self.plasma_osmolality <= 400.0):
            raise ValueError(f"plasma_osmolality {self.plasma_osmolality} mOsm/kg outside valid range 250-400")
        if self.gill_na_k_atpase is not None and not (1.0 <= self.gill_na_k_atpase <= 20.0):
            raise ValueError(f"gill_na_k_atpase {self.gill_na_k_atpase} outside valid range 1-20")
        if self.drinking_rate is not None and not (0.0 <= self.drinking_rate <= 50.0):
            raise ValueError(f"drinking_rate {self.drinking_rate} mL/hr/kg outside valid range 0-50")
