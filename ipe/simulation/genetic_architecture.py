...
class GeneticArchitecture:
    ...
    def calculate_breeding_values(self, genotype: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        for trait_name, trait_arch in self.traits.items():
            ...
            for locus_name in trait_arch.contributing_loci:
                if locus_name in genotype and locus_name in self.loci:
                    ...
                    effect_size = self.loci[locus_name].trait_effects.get(
                        trait_name, 0.0
                    )
    ...
    def calculate_phenotype(
        ...
        environment: Optional[PhysiologicalState] = None,
    ) -> Dict[str, float]:
        """
    ...
    def get_genetic_correlations(self) -> np.ndarray:
        ...
        mut_variances = np.array(
            [self.loci[locus_name].allelic_variance for locus_name in self.loci.keys()]
        )
        ...
        correlations = np.divide(
            G_cov, correlations, out=np.zeros_like(G_cov), where=correlations != 0
        )
