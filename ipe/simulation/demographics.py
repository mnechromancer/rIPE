...
class Demographics:
    ...
    def get_age_structure(self) -> List[AgeClass]:
        ...
        for age, individuals in sorted(age_groups.items()):
            ...
            age_class = AgeClass(
                age=age, count=len(individuals), mean_fitness=mean_fitness
            )
    ...
    def calculate_survival_rates(
        self, previous_age_structure: List[AgeClass]
    ) -> List[AgeClass]:
        """
        ...
        prev_lookup = {
            ac.age - 1: ac.count for ac in previous_age_structure if ac.age > 0
        }
    ...
    def track_demographic_trajectory(self, generations: int) -> List[Dict[str, Any]]:
        """
