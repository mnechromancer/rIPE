...
class MutationStrategy(ABC):
    ...
    @abstractmethod
    def mutate(
        self, individual: Individual, architecture: GeneticArchitecture
    ) -> Individual:
        """
...
class GaussianMutation(MutationStrategy):
    ...
    def mutate(
        ...
        mutated_individual = Individual(
            ...
            genetic_values=(
                individual.genetic_values.copy() if individual.genetic_values else {}
            ),
            age=individual.age,
        )
        ...
        for locus_name, locus in architecture.loci.items():
            if np.random.random() < locus.mutation_rate:
                ...
                mutated_individual.genetic_values[locus_name] = (
                    current_value + effect_size
                )
    ...
    def mutate(
        ...
        if n_loci > 0:
            ...
            for i, locus_name in enumerate(locus_names):
                ...
                if np.random.random() < locus.mutation_rate:
                    current_value = mutated_individual.genetic_values.get(
                        locus_name, 0.0
                    )
                    mutated_individual.genetic_values[locus_name] = (
                        current_value + mutations[i]
                    )
...
class MutationEngine:
    ...
    def evolve_mutation_rates(self, population: List[Individual]) -> None:
        ...
        for locus_name in self.architecture.loci.keys():
            ...
            rate_fitness_effect = (
                -self.parameters.mutation_rate_selection_strength * rate_deviation**2
            )
    ...
    def maintain_standing_variation(
        self, population: List[Individual], target_variance: float = 0.01
    ) -> List[Individual]:
        """
        ...
        for locus_name in self.architecture.loci.keys():
            genetic_values = [
                ind.genetic_values.get(locus_name, 0.0) for ind in population
            ]
            ...
            if current_variance < target_variance:
                ...
                for individual in population:
                    if np.random.random() < 0.1:  # 10% chance per individual
                        ...
                        individual.genetic_values[locus_name] = (
                            current_value + mutation_effect
                        )
    ...
    def get_mutation_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "mutation_rate_variance": np.var(
                list(self.current_mutation_rates.values())
            ),
        ...
        if self.effect_sizes:
            stats["mean_effect_sizes"] = {
                locus: np.mean(effects) for locus, effects in self.effect_sizes.items()
            }
            stats["effect_size_variances"] = {
                locus: np.var(effects) for locus, effects in self.effect_sizes.items()
            }
...
def create_default_mutation_engine(architecture: GeneticArchitecture) -> MutationEngine:
    ...
    """
    parameters = MutationParameters(
        base_mutation_rate=1e-4, mutational_variance=0.01, pleiotropic_correlation=0.1
    )
