...
class SelectionStrategy(ABC):
    ...
    @abstractmethod
    def select(self, individuals: List[Individual], num_survivors: Optional[int] = None) -> List[Individual]:
        """
    ...
    def calculate_selection_differential(self, 
                                       ...
                                       trait_name: str = "fitness") -> SelectionDifferential:
        """
...
class FrequencyDependentSelection(SelectionStrategy):
    ...
    def select(self, individuals: List[Individual], num_survivors: Optional[int] = None) -> List[Individual]:
        ...
        frequencies = {trait: count/len(traits) for trait, count in zip(unique_traits, counts)}
...
class MultiTraitSelection(SelectionStrategy):
    ...
    def __init__(self, 
        ...
        self.trait_weights = trait_weights or {name: 1.0 for name in trait_extractors.keys()}
    ...
    def select(self, individuals: List[Individual], num_survivors: Optional[int] = None) -> List[Individual]:
        ...
        for individual in individuals:
            ...
            individual.fitness = composite_fitness / total_weight if total_weight > 0 else 0.0
        
        # Use truncation selection with new fitness values
        truncation_selector = TruncationSelection(survival_fraction=num_survivors/len(individuals))
...
class SelectionAnalyzer:
    ...
    @staticmethod
    def analyze_selection(before: List[Individual], 
                         after: List[Individual]) -> Dict[str, Any]:
        """
        ...
        selection_diff = after_mean - before_mean
        selection_intensity = selection_diff / np.sqrt(before_var) if before_var > 0 else 0.0
