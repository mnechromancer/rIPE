...
class TestAllometricScaling:
    ...
    def test_kleiber_metabolic_scaling(self, mass_range, reference_data):
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(log_mass, log_bmr)
        ...
        assert p_value < 0.001, f"Scaling relationship not significant: p = {p_value:.3f}"
        
        print(f"✅ Kleiber's law validated: BMR ∝ M^{slope:.3f} (R² = {r_value**2:.3f})")
    ...
    def test_heart_rate_scaling(self, mass_range, reference_data):
        ...
        print(f"✅ Heart rate scaling validated: HR ∝ M^{slope:.3f} (R² = {r_value**2:.3f})")
    ...
    def test_lifespan_scaling(self, mass_range, reference_data):
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(log_mass, log_lifespan)
        ...
        print(f"✅ Lifespan scaling validated: Lifespan ∝ M^{slope:.3f} (R² = {r_value**2:.3f})")
    ...
    def test_surface_area_scaling(self, mass_range, reference_data):
        ...
        print(f"✅ Surface area scaling validated: SA ∝ M^{slope:.3f} (R² = {r_value**2:.3f})")
    ...
    def test_organ_mass_scaling(self, reference_data):
        ...
        for organ, ref in organ_refs.items():
            organ_masses = [scaler.scale_organ_mass(organ, mass) for mass in test_masses]
            
            # Fit power law
            log_mass = np.log10(test_masses)
            log_organ = np.log10(organ_masses)
            
            slope, intercept, r_value, p_value, std_err = stats.linregress(log_mass, log_organ)
            ...
            assert r_value**2 > 0.95, (
                f"{organ.capitalize()} poor fit: R² = {r_value**2:.3f}"
            )
            
            print(f"✅ {organ.capitalize()} scaling validated: "
    ...
    def test_metabolic_scaling_temperature_independence(self):
        ...
        for temp in temperatures:
            bmr_values = [calculator.basal_metabolic_rate(mass, temp_c=temp) 
        ...
        assert abs(mean_exponent - 0.75) <= 0.05, (
            f"Mean scaling exponent {mean_exponent:.3f} differs from 0.75"
        )
        
        print(f"✅ Temperature independence validated: "
    ...
    def test_evolutionary_constraints_scaling(self):
        ...
        for mass in body_masses:
            ...
            max_heart_ratio = scaler.scale_organ_mass("heart", mass * 1.5) / baseline_heart
            ...
            heart_mass_cost = (adapted_heart - baseline_heart) * 20  # 20 W/kg heart tissue
        ...
        print(f"✅ Evolutionary constraints validated for {len(body_masses)} size classes")
    ...
    def test_scaling_relationship_consistency(self):
        ...
        print(f"✅ Scaling consistency validated: BMR-CO correlation = {correlation:.3f}")
