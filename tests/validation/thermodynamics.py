...
try:
    ...
    class EnergyBudget:
        ...
        def is_balanced(self, tolerance=0.01):
            return (
                abs(self.input_energy - self.output_energy - self.stored_energy)
                < tolerance
            )
...
class TestThermodynamicConsistency:
    ...
    def test_energy_conservation_principle(
        self, sample_organism, environmental_conditions
    ):
        ...
        metabolic_input = calculator.basal_metabolic_rate(
            sample_organism["mass_kg"], environmental_conditions["ambient_temp_c"]
        )
        ...
        assert result[
            "valid"
        ], f"Energy conservation violated: error = {result['error']}"
        assert (
            result["error"] < 0.01
        ), f"Energy balance error too large: {result['error']}"
    ...
    def test_metabolic_efficiency_limits(self, thermodynamic_data):
        ...
        for condition in test_conditions:
            efficiency = calculator.calculate_efficiency(
                temperature=condition["temp_c"], activity_level=condition["activity"]
            )
    ...
    def test_heat_balance_equation(self, sample_organism, environmental_conditions):
        ...
        heat_loss = heat_transfer.calculate_heat_loss(
            organism=sample_organism, environment=environmental_conditions
        )
    ...
    def test_entropy_increase_constraint(self):
        ...
        processes = [
            {"name": "metabolism", "expected_delta_s": 0.1},
            {
                ...
                "expected_delta_s": -0.05,
            },  # Negative OK if coupled
    ...
    def test_allometric_scaling_laws(self, thermodynamic_data):
        ...
        print(
            f"✅ Allometric scaling validated: exponent = {scaling_exponent:.3f} "
    ...
    def test_temperature_dependence_arrhenius(self):
        ...
        print(
            f"✅ Arrhenius temperature dependence validated: "
