    ...
    class IPEClient:
        ...
        def get_simulation_results(self, simulation_id):
            return {
                ...
                "outputs": {
                    "visualization_urls": ["/viz/fitness_trajectory.png", "/viz/trait_evolution.png"],
...
class TestEndToEndSimulationFlow:
    ...
    def high_altitude_adaptation_config(self):
        ...
        return {
            ...
            "measurements": [
                "fitness", "hematocrit", "heart_mass", "metabolic_rate",
                "body_mass", "survival_rate"
            ],
            "outputs": {
                "visualizations": ["fitness_trajectory", "trait_evolution", "population_dynamics"],
                "exports": ["json", "csv", "publication_figures"],
                "analysis": ["statistical_summary", "evolutionary_trends", "adaptation_rates"]
            }
        }
    ...
    def thermal_physiology_config(self):
        ...
        return {
            ...
            "measurements": [
                "thermal_performance", "critical_thermal_max", "metabolic_scope",
                "behavioral_thermoregulation", "heat_shock_response"
            ],
            "analysis": {
    ...
    def test_complete_research_workflow(self, client, high_altitude_adaptation_config):
        ...
        assert status["status"] == "completed", f"Simulation didn't complete in {max_wait_time}s"
        ...
        assert adaptation_metrics["hematocrit_change"] > 0, "Expected hematocrit increase"
        assert adaptation_metrics["heart_mass_change"] > 0, "Expected heart mass increase"
        
        # Step 5: Generate analysis and outputs
        analysis_config = {
            "simulation_id": simulation_id,
            "analysis_types": ["evolutionary_trends", "adaptation_rates", "fitness_landscapes"],
        ...
        for expected in expected_viz:
            assert any(expected in url for url in viz_urls), (
                f"Missing visualization: {expected}"
            )
        
        print(f"✅ Complete research workflow validated: "
    ...
    def test_multi_scenario_comparative_study(self, client):
        ...
        for scenario in scenarios:
            config = {
                "organism": {"species": "Ochotona princeps", "initial_population_size": 500},
            ...
            status = client.get_simulation_status(simulation_id)
            assert status["status"] == "completed", f"Scenario {scenario['name']} failed"
        ...
        for name, result in simulation_results.items():
            ...
            hematocrit_changes[name] = result["results"]["adaptation_metrics"]["hematocrit_change"]
        ...
        assert moderate_fitness >= severe_fitness * 0.8, (
            "Unrealistic fitness pattern across oxygen levels"
        )
        
        print(f"✅ Multi-scenario study validated: {len(scenarios)} scenarios, "
    ...
    def test_interactive_dashboard_workflow(self):
        ...
        with patch('requests.post') as mock_post:
            ...
            response = requests.post(
                "http://localhost:8080/api/simulations/start",
    ...
    def test_performance_stress_test(self, client):
        ...
        assert len(completion_times) == num_concurrent, (
        ...
        assert avg_completion_time < max_reasonable_time, (
            f"Average completion time too slow: {avg_completion_time:.1f}s"
        )
        
        print(f"✅ Performance stress test validated: "
    ...
    def test_data_export_integration(self, client):
        ...
        print(f"✅ Data export integration validated: "
