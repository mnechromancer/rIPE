...
def test_epic6_backlog_update_mechanism():
    ...
    viz_stories = [story_id for story_id in stories.keys() if story_id.startswith('VIZ-')]
    ...
    completed_viz = [s for s_id, s in stories.items() if s_id.startswith('VIZ-') and s.status == 'done']
    todo_viz = [s for s_id, s in stories.items() if s_id.startswith('VIZ-') and s.status == 'todo']
    ...
    assert len(completed_viz) > 0, "At least one VIZ story should be marked as completed (demonstrating backlog updates)"
