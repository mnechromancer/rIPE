...
from ipe.core.plasticity.assimilation import (
    GeneticAssimilationEngine, AssimilationTrajectory, AssimilationStage
)
from ipe.core.plasticity.canalization import (
    CanalizationEngine, CanalizationTrajectory, CanalizationMeasure, CanalizationType
)
...
class TestGeneticAssimilationEngine:
    ...
    def test_calculate_assimilation_rate(self):
        ...
        rate_high = engine.calculate_assimilation_rate(norm, target_environment=20.0, selection_coefficient=0.5)
        rate_low = engine.calculate_assimilation_rate(norm, target_environment=20.0, selection_coefficient=0.01)
    ...
    def test_simulate_assimilation_trajectory(self):
        ...
        trajectory = engine.simulate_assimilation_trajectory(norm, target_environment=25.0, generations=100)
    ...
    def test_predict_time_to_assimilation(self):
        ...
        time = engine.predict_time_to_assimilation(norm, target_environment=20.0, threshold_plasticity=5.0)
        ...
        time_impossible = engine.predict_time_to_assimilation(norm, target_environment=20.0, threshold_plasticity=0.001)
        
        # Test with high threshold (should be immediate)
        time_immediate = engine.predict_time_to_assimilation(norm, target_environment=20.0, threshold_plasticity=50.0)
    ...
    def test_assess_assimilation_stage(self):
        ...
        stage_initial = engine.assess_assimilation_stage(current_plasticity=95.0, initial_plasticity=100.0)
        assert stage_initial == AssimilationStage.INITIAL
        
        stage_partial = engine.assess_assimilation_stage(current_plasticity=70.0, initial_plasticity=100.0)
        assert stage_partial == AssimilationStage.PARTIAL
        
        stage_advanced = engine.assess_assimilation_stage(current_plasticity=40.0, initial_plasticity=100.0)
        assert stage_advanced == AssimilationStage.ADVANCED
        
        stage_complete = engine.assess_assimilation_stage(current_plasticity=10.0, initial_plasticity=100.0)
        assert stage_complete == AssimilationStage.COMPLETE
        
        stage_canalized = engine.assess_assimilation_stage(current_plasticity=1.0, initial_plasticity=100.0)
        assert stage_canalized == AssimilationStage.CANALIZED
        
        # Test edge case
        stage_zero = engine.assess_assimilation_stage(current_plasticity=50.0, initial_plasticity=0.0)
    ...
    def test_compare_assimilation_scenarios(self):
        ...
        comparisons = engine.compare_assimilation_scenarios(norm, environments, generations=50)
    ...
    def test_predict_evolutionary_endpoint(self):
        ...
        endpoint = engine.predict_evolutionary_endpoint(norm, target_environment=25.0, max_generations=100)
        
        assert isinstance(endpoint, dict)
        
        required_keys = [
            'final_plasticity', 'final_constitutive_value', 'assimilation_stage',
            'canalization_strength', 'time_to_95_percent_reduction', 'predicted_stable'
        ]
        ...
        assert isinstance(bool(endpoint['predicted_stable']), bool)  # Convert numpy bool
        assert endpoint['assimilation_stage'] in [stage.value for stage in AssimilationStage]
...
class TestCanalizationEngine:
    ...
    def test_predict_canalization_trajectory(self):
        ...
        assert can_trajectory.canalization_strength[0] <= can_trajectory.canalization_strength[-1]
    ...
    def test_compare_canalization_types(self):
        ...
        comparison = engine.compare_canalization_types(norms, environment=20.0, developmental_variance=2.0)
        ...
        single_comparison = engine.compare_canalization_types([norms[0]], environment=20.0)
    ...
    def test_assess_canalization_evolution(self):
        ...
        required_keys = [
            'initial_canalization', 'final_canalization', 'max_canalization',
            'mean_canalization', 'canalization_evolution_rate', 'final_variance_reduction',
            'evolutionary_stability', 'canalization_efficiency'
        ]
    ...
    def test_identify_canalization_drivers(self):
        ...
        expected_keys = ['selection', 'genetic_drift', 'mutation', 'developmental_buffering']
        for key in expected_keys:
...
class TestIntegrationAssimilationCanalization:
    ...
    def test_comparative_analysis(self):
        ...
        environments = [10.0, 20.0, 30.0]
        trajectories = assim_engine.compare_assimilation_scenarios(norm, environments, generations=50)
        ...
        assert len(set([round(fp, 1) for fp in final_plasticities])) >= 1  # Some variation expected
