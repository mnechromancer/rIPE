...
class TestSableSessionData:
    ...
    def test_session_properties(self):
        ...
        assert session.duration_minutes == pytest.approx(59/60, abs=0.01)  # 59 seconds
    ...
    def create_test_csv_file(self, filename="test.csv", n_points=10):
        ...
        with open(filepath, 'w') as f:
            ...
            for i in range(n_points):
                ...
                f.write(f"{timestamp},{vo2:.3f},{vco2:.3f},{temp:.1f},{humidity:.1f},{pressure:.1f},{flow:.1f}\n")
...
class TestGenericRespirometryParser:
    ...
    def test_filter_measurements(self):
        ...
        filtered = self.parser.filter_measurements(
            session, min_vo2=2.0, max_vo2=3.0)
...
class TestQualityAssessment:
        ...
        if quality_type == "good":
            ...
            for i in range(30):  # Very short
                measurement = RespirometryMeasurement(
                    ...
                    rer=max(0.1, min(2.0, 0.8 + 0.2 * np.random.randn())),  # Variable but valid RER
    ...
    def test_quality_recommendations(self):
        ...
        assert any(keyword in rec_text.lower() for keyword in ['smoothing', 'temperature', 'filtering', 'validation'])
