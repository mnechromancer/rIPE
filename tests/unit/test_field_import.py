...
class TestEnvironmentalDataImporter:
            ...
            data.append(
                {
                    ...
                    "precipitation": (
                        np.random.exponential(0.1) if np.random.rand() < 0.2 else 0
                    ),
                }
            )
    ...
    def test_single_station_import(self):
        ...
        dataset = self.importer.import_weather_station_csv(
            filepath, "TEST_STATION", "test_dataset"
        )
    ...
    def test_multiple_stations_import(self):
        ...
        dataset = self.importer.import_multiple_stations(
            self.temp_dir, "multi_station_test"
        )
    ...
    def test_gps_alignment(self):
        ...
        dataset = self.importer.import_weather_station_csv(
            weather_file, "WEATHER_STATION", "alignment_test"
        )
    ...
    def test_data_interpolation(self):
        ...
        dataset = self.importer.import_weather_station_csv(
            weather_file, "GAP_STATION", "gap_test"
        )
        ...
        interpolated_temps = [
            r.temperature_c
            for r in interpolated.readings
            if r.temperature_c is not None
        ]
        assert len(interpolated_temps) > len(
            [r.temperature_c for r in dataset.readings if r.temperature_c is not None]
        )
    ...
    def test_dataset_queries(self):
        ...
        dataset = self.importer.import_weather_station_csv(
            weather_file, "QUERY_STATION", "query_dataset"
        )
    ...
    def test_export_csv(self):
        ...
        dataset = self.importer.import_weather_station_csv(
            weather_file, "EXPORT_STATION", "export_dataset"
        )
    ...
    def test_export_json(self):
        ...
        dataset = self.importer.import_weather_station_csv(
            weather_file, "JSON_STATION", "json_dataset"
        )
...
class TestMorphologyDataImporter:
    ...
    def test_quality_control(self):
        ...
        outlier_measurement = MorphologicalMeasurement(
            measurement_id="OUTLIER",
            specimen_id=specimens_list[1].specimen_id,
            trait_name="body_mass",
            value=1000.0,
            unit="g",
...
class TestIntegration:
    ...
    def test_full_field_data_workflow(self):
        ...
        morph_dataset = morph_importer.import_specimen_csv(
            specimen_file, "field_morphometry"
        )
...
class TestPerformance:
    ...
    def test_large_environmental_dataset(self):
        ...
        dataset = importer.import_weather_station_csv(
            large_file, "LARGE_STATION", "performance_test"
        )
