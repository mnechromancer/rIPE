...
class TestGeneticLocus:
    ...
    def test_creation_errors(self):
        ...
        with pytest.raises(
            ValueError, match="Dominance coefficient must be between -1 and 1"
        ):
...
class TestTraitArchitecture:
    ...
    def test_creation_errors(self):
        ...
        with pytest.raises(
            ValueError, match="Environmental variance must be non-negative"
        ):
            TraitArchitecture(
                "test", TraitType.CONTINUOUS, [], environmental_variance=-1.0
            )
...
class TestGeneticArchitecture:
    ...
    def sample_architecture(self):
        ...
        traits = [
            ...
            TraitArchitecture(
                "trait2", TraitType.CONTINUOUS, ["locus1", "locus2", "locus3"]
            ),
        ]
    ...
    def test_calculate_phenotype(self, sample_architecture):
        ...
        assert (
            abs(phenotypes["trait2"] - 0.2) < 1.0
        )  # Should be around 0.2 ± environmental variance
...
class TestMutationParameters:
    ...
    def test_creation_errors(self):
        ...
        with pytest.raises(
            ValueError, match="Mutational variance must be non-negative"
        ):
            MutationParameters(mutational_variance=-1.0)

        with pytest.raises(
            ValueError, match="Pleiotropic correlation must be between 0 and 1"
        ):
...
class TestGaussianMutation:
    ...
    def sample_architecture(self):
        ...
        loci = [
            GeneticLocus(
                "locus1", {"trait1": 0.5}, mutation_rate=1.0, allelic_variance=0.1
            ),  # High rate for testing
            GeneticLocus(
                "locus2", {"trait1": 0.3}, mutation_rate=0.0, allelic_variance=0.1
            ),  # No mutation
        ]
        for locus in loci:
            arch.add_locus(locus)
...
class TestMutationEngine:
    ...
    def sample_population(self):
        ...
        for i in range(5):
            individual = Individual(
                ...
                physiological_state=state,
                genetic_values={
                    "locus1": np.random.normal(0, 1),
                    "locus2": np.random.normal(0, 1),
                },
            )
    ...
    def test_maintain_standing_variation(self, sample_population):
        ...
        np.random.seed(42)
        maintained_pop = engine.maintain_standing_variation(
            sample_population, target_variance=0.01
        )
