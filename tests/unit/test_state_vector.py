...
class TestPhysiologicalState:
    ...
    def test_validation_environmental_errors(self):
        ...
        with pytest.raises(ValueError, match="salinity.*outside valid range"):
            state = PhysiologicalState(po2=15.0, temperature=25.0, altitude=1000.0, salinity=40.0)
            state.validate()
    ...
    def test_validation_cardiovascular_errors(self):
        ...
        with pytest.raises(ValueError, match="heart_mass.*outside valid range"):
            state = PhysiologicalState(po2=15.0, temperature=25.0, altitude=1000.0, heart_mass=20.0)
            state.validate()
            
        with pytest.raises(ValueError, match="hematocrit.*outside valid range"):
            state = PhysiologicalState(po2=15.0, temperature=25.0, altitude=1000.0, hematocrit=80.0)
            state.validate()
            
    def test_validation_temperature_logic(self):
        """Test validation catches temperature logic errors"""
        with pytest.raises(ValueError, match="lower_critical_temp.*must be.*upper_critical_temp"):
            state = PhysiologicalState(
                po2=15.0, 
                temperature=25.0, 
                altitude=1000.0,
    ...
    def test_validation_mitochondrial_density(self):
        ...
        with pytest.raises(ValueError, match="mitochondrial_density.*outside valid range"):
            state = PhysiologicalState(
                po2=15.0,
                temperature=25.0,
                altitude=1000.0,
