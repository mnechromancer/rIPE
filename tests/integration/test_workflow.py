...
try:
    ...
    class WorkflowManager:
        ...
        def run_full_workflow(self, config):
            self.steps_completed = [
                "data_import", "preprocessing", "simulation", 
...
class TestWorkflowIntegration:
    ...
    def test_full_workflow_execution(self, workflow_config):
        ...
        print(f"✅ Full workflow completed in {execution_time:.1f}s with {len(completed_steps)} steps")
    ...
    def test_workflow_error_handling(self, workflow_config):
        ...
        invalid_configs = [
            {**workflow_config, "simulation": {"population_size": -1}},  # Negative population
            {**workflow_config, "environment": {"temperature_range": [50, 10]}},  # Invalid range
            {**workflow_config, "data_sources": {"field_data": "nonexistent.json"}}  # Missing file
        ]
        ...
        for i, config in enumerate(invalid_configs):
            with patch.object(WorkflowManager, 'run_full_workflow') as mock_workflow:
                ...
                assert result["status"] == "error", f"Should have failed for config {i+1}"
                assert "error" in result, f"No error message for config {i+1}"
                
                # Should not complete all steps on error
                assert len(result["steps"]) < 6, f"Too many steps completed on error {i+1}"
        
        print(f"✅ Error handling validated for {len(invalid_configs)} invalid configurations")
    ...
    def test_data_import_integration(self, mock_data_files):
        ...
        print(f"✅ Data import integration validated: "
    ...
    def test_simulation_analysis_pipeline(self):
        ...
        for scenario in test_scenarios:
            ...
            assert "mean_fitness" in sim_results, f"No fitness data for {scenario['name']}"
            assert "final_generation" in sim_results, f"No generation data for {scenario['name']}"
            
            # Fitness should be reasonable (0-1 range)
            fitness = sim_results["mean_fitness"]
            assert 0 <= fitness <= 1, f"Invalid fitness {fitness} for {scenario['name']}"
            
            results.append(result)
        
        # Compare results across scenarios
        fitnesses = [r["results"]["mean_fitness"] for r in results]
        assert len(set(fitnesses)) > 1, "All scenarios produced identical fitness values"
        
        print(f"✅ Simulation-analysis pipeline validated for {len(test_scenarios)} scenarios")
    ...
    def test_concurrent_workflow_execution(self):
        ...
        configs = [
            {"simulation": {"population_size": 100, "generations": 20}, "id": "workflow_1"},
            {"simulation": {"population_size": 200, "generations": 30}, "id": "workflow_2"},
            {"simulation": {"population_size": 150, "generations": 25}, "id": "workflow_3"}
        ]
        ...
        print(f"✅ Concurrent execution validated: {len(results)} workflows, "
    ...
    def test_output_format_consistency(self):
        ...
        for format_type in output_formats:
            assert format_type in mock_data_checksums, (
        ...
        assert len(mock_data_checksums) == len(output_formats), (
            "Number of output formats doesn't match request"
        )
        
        print(f"✅ Output format consistency validated for {len(output_formats)} formats")
