...
try:
    ...
    class DataProcessor:
        def clean_data(self, data):
            return {
                "cleaned_records": len(data.get("records", [])),
                "outliers_removed": 5,
            }

        def normalize_data(self, data):
            return {
                "normalized_records": data.get("cleaned_records", 0),
                "scaling": "z_score",
            }
...
class TestDataPipelineIntegration:
    ...
    def data_schema(self):
        ...
        return {
            ...
            "field_data": {
                ...
                "ranges": {"mass_g": [80, 300], "body_length_mm": [120, 200]},
            },
        }

    @pytest.mark.integration
    def test_complete_data_pipeline(
        self, sample_respirometry_data, sample_field_data, data_schema
    ):
        ...
        resp_validation = validator.validate_schema(
            resp_result, data_schema["respirometry"]
        )
        field_validation = validator.validate_schema(
            field_result, data_schema["field_data"]
        )

        assert resp_validation[
            "valid"
        ], f"Respirometry validation failed: {resp_validation['errors']}"
        assert field_validation[
            "valid"
        ], f"Field data validation failed: {field_validation['errors']}"
        ...
        assert (
            "normalized_records" in resp_normalized
        ), "No normalization results for respirometry"
        ...
        for i, result in enumerate(export_results):
            ...
            assert (
                result["exported_records"] > 0
            ), f"No records exported to {format_type}"

        print(
            f"✅ Complete data pipeline validated: "
    ...
    def test_data_quality_monitoring(self, sample_respirometry_data):
        ...
        assert (
            initial_quality["quality_score"] > 0.8
        ...
        assert (
            post_cleaning_quality["quality_score"] >= initial_quality["quality_score"]
        ...
        assert (
            post_missing <= initial_missing
        ), f"Missing values increased: {initial_missing} -> {post_missing}"

        print(
            f"✅ Data quality monitoring validated: "
    ...
    def test_data_lineage_tracking(self):
        ...
        with patch.object(DataProcessor, "clean_data") as mock_clean:
            mock_clean.return_value = {
                **mock_data,
                "lineage": {**mock_data["lineage"], "operations": ["clean_data"]},
            }
            ...
            assert (
                "clean_data" in cleaned["lineage"]["operations"]
            ), "Clean operation not logged"
        ...
        print(
            f"✅ Data lineage tracking validated: {len(operations)} operations tracked"
        )
        ...
        with patch.object(DataImportManager, "import_respirometry") as mock_import:
            ...
            try:
                ...
                if result["status"] == "error":
                    # Retry once
                    result = import_manager.import_respirometry(
                        sample_respirometry_data
                    )
            except Exception:
    ...
    def test_large_dataset_pipeline(self):
        ...
        with patch.object(DataImportManager, "import_respirometry") as mock_import:
            ...
            assert (
                result["records"] == large_dataset_size
            ), "Large dataset import failed"
            assert (
                import_time < 5.0
            ), f"Import too slow: {import_time:.2f}s for {large_dataset_size} records"
    ...
    def test_multi_format_export_consistency(self):
        ...
        print(
            f"✅ Multi-format export consistency validated: "
