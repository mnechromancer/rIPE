...
def alpine_fitness_function(individual: Individual, environment: PhysiologicalState) -> float:
    """
    ...
    altitude_stress = abs(individual.physiological_state.altitude - environment.altitude) / 1000.0
    temp_stress = abs(individual.physiological_state.temperature - environment.temperature) / 10.0
...
def main():
    ...
    initial_population = Population(
        size=150,  # Your population_size parameter
        initial_state=sea_level_state
    )
    ...
    mutation_engine = MutationEngine(
        ...
        architecture=genetic_architecture
    )
    
    # Set up selection strategy (moderate selection pressure)
    print("🎯 Setting up selection strategy...")
    selection_strategy = TruncationSelection(survival_fraction=0.7)  # 70% survive each generation
    ...
    for generation_stats in simulator.simulate_constant_environment(
        environment=alpine_environment,
        generations=25  # Your duration parameter
    ):
        ...
        if gen % 5 == 0 or gen == 1:
            print(f"Gen {gen:2d}: Pop={pop_size:3d}, Mean Fitness={mean_fitness:.3f}, Max Fitness={max_fitness:.3f}")
    ...
    print(f"   Fitness improvement:  {final_stats['mean_fitness'] - initial_stats['mean_fitness']:.3f}")
    print(f"   Final population:     {final_stats['population_size']}")
    print(f"   Selection differential: {final_stats.get('selection_differential', 'N/A')}")
    ...
    print("   The population adapted to high-altitude, cold conditions over 25 generations.")
